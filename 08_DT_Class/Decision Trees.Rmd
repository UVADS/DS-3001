...
features <- train[,-12]#dropping 13 because it's target variable. 
View(features)
...
wine_pred_tune = predict(wine_mdl,tune,tune$text_rank, type= "prob")
...
total.sulfur.dioxide <- cut(winequality$total.sulfur.dioxide,c(0,32,300), labels = c("low","high"))
...
train_1 <- winequality[part_index_1, ]
tune_and_test_1 <- winequality[-part_index_1, ]
...
predictandCM(wine_mdl_2,tune,"raw",tune$text_rank)
...
```{r}
predictandCM(wine_mdl,test,"raw",tune$text_rank)
predictandCM(wine_mdl_1,test,"raw",tune$text_rank)
predictandCM(wine_mdl_2,test,"raw",tune$text_rank)
```

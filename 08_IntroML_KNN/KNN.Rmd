...
ACCURACY IS HIGH 87.15%, BUT THE SENSITIVITY IS VERY LOW (29.82%) THIS MEANS THAT IT IS NOT AS GOOD AT PREDICTING THE POSITIVE CLASS 

GOAL = MAXIMIZE SENSITIVITY -> WANT TO GET THE HIGHEST PROBABILITY OF IDENTIFYING PEOPLE WHO ARE MOST LIKELY TO SIGN UP 
...
#CREATING A FUNCTION TO DEVELOP ELBOW CHART DATA
...
BECAUSE DATA SET IS UNBALANCED IT LEARNED THE MAJORITY CLASS VERY QUICKLY AND INCREASING COMPLEXITY DOES NOT IMPACT THE ACCURACY. WILL LIKELY NEED TO OVERSAMPLE THE MINORITY CLASS TO IMPROVE MODEL PERFORMANCE IN SENSITIVITY.
...
bank_5NN <-  knn(train = train[, c("age", "balance", "duration")],#<- training set cases
               ...
               use.all = TRUE,
               prob = TRUE) #<- control ties between class assignments If true, all distances equal to 
...
adding complexity does not always improve - it actually showed that the sensitivity decreased 
instead you need to over-sample to create a more balanced dataset

do you need robust training endeavor? -> memorizes and puts model into working memory, therefore does not make sense to train because you do not actually end with a model object (you train and test on deployment - they are linked)
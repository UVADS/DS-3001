...
clust_data_Dem = house_votes_Dem[, c("aye", "nay", "other")] #3 continuous variables, and segment them off to use
#using column names as indicators, or could index but this works

#don't need to normalize data but 'aye', 'nay', and 'other' are all on the same scale 
...
#lloyd means euclidean distance

# What did the kmeans function produce, 
# what does the new variable kmeans_obj contain?
kmeans_obj_Dem

#total sum of squares - with in difference/total variance 
...
house_votes_color_Dem = inner_join(house_votes_Dem, party_color3D_Dem) #joining color into data frame

house_votes_color_Dem$clusters <- (party_clusters_Dem)

str(house_votes_color_Dem)

house_votes_color_Dem$Last.Name <- gsub("[^[:alnum:]]", "", house_votes_color_Dem$Last.Name) #take special characters out of last name--to avoid can't parse string error
...
#sapply() is similar to a nested for loop 
...
#as complexity increases, variance goes down -- trade off between complexity and accuracy 
#==================================================================================

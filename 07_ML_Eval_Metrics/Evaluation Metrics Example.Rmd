...
loan_pred <- as.tibble(loan_pred)#converting back to a tibble for the normalizing function
...
So now we can create our decision tree, it's important to note that the outputs of most ML models are essentially the same. 
...
confusionMatrix(loan_eval$pred_class, test$outcome, positive = "1", dnn=c("Prediction", "Actual"), mode = "everything")
...
#We want this number to be rather close to 0, but compare to our baseline which is .45 so this is a pretty terrible result. 
...
View(pred_1)
F1_Score(y_pred = loan_eval$pred_class, y_true = loan_eval_prob$test, positive = "1")

...
#Just have blob of text (one cell), must then take out special characters and then carve into pieces (words or sequences of words) and use pieces to find structure
ah_word <- animal_house %>%
  unnest_tokens(word, text) 
#unnest_tokens, unnest by word and use the text data
...
#Will do it by every 3 words and then break - single words is most common use case - typically for the exploratory phase you need to start with single words
...
ah_count <- ah_word %>%
  count(word, sort=TRUE) #get frequency from word tokenization -> will sort from most to least

#stop words = "the" and can take out for certain analysis
...
#STOP WORDS ARE A LEXICON WHICH YOU CAN REMOVE WITH ANTIJOIN
#YOU TAKE OUT ALL WORDS THAT ARE SIMILAR
ah_word_sw <- ah_word %>% #take out tokenized list
...
#only keeps the word in the lexicon therefore will reduce the space
...
#CORPUS IS COLLECTION OF SPEECHES
#DOCUMENT IS INDIVIDUAL SPEECHES
#TOKENIZATION IS BREAKING IT DOWN ON A WORD BY WORD BASIS

#TF IDF SHOWS THE RELATIVE IMPORTANCE OF A WORD IN A DOCUMENT COMPARED TO THE CORPUS
#HIGHER NUMBER MEANS THAT THAT WORD IS MORE IMPORTANT TO A PARTICULAR DOCUMENT COMPARED TO THE REST OF THE BODY
...
biden_inag_raw <- as.tibble(read_lines("biden_inag.txt"))

#transposing so they are in columns next to each other - join column wise
...
#pass in data, pass in range V1 to V131
#corpus merges all the lines into one cell
...
#KEEP IN THE STOP WORDS - YOU WANT TO HAVE FULL RATIO TO TRACK MOST IMPORTANT WORDS ACCURATELY

#SINCE STOP WORDS ARE PREVALENT IN EVERY SPEECH THEY HAVE LOW TF AND IDF VALUES

View(inag_words)
#WHAT ARE THE TOP 10 WORDS IN EACH CORPUS?
